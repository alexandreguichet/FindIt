...
filtered, final, lines = hough_transform(image, ad)
lines = lines.reshape(-1, 4)
lines = lines[((lines[:,1] < ind[0][2] ) & (lines[:,1] > ind[0][1]) & (lines[:,3] < ind[0][2]) & (lines[:,3] > ind[0][1]))]
...
lines_class = [Line(i) for i in all_lines]

i = 0
for l in lines_class[10:11]:           # find intersections
    g = l.root()
...
#     remove_list_l = False  #is line (l) in a group
#     line_group = False;
#     to_remove = list()
